...
async def start(event):
  await event.reply("**Karma Tagger Bot**, Grup vəya kanaldaki demək olarki bütün üyeleri tag edə bilərəm ★\nDaha ətraflı məlumat üçün **/help**'i tıklayın.",
                    buttons=(
                      [Button.url('🌟 Məni Qrupa əlavə et', 'https://t.me/KarmaTagger_bot?startgroup=a'),
...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("__Bu komut gruplarda və kanallarda işlədilə bilər.!__")
  
  admins = []
  async for admin in client.iter_participants(event.chat_id, filter=ChannelParticipantsAdmins):
    admins.append(admin.id)
  if not event.sender_id in admins:
    return await event.respond("__Yalnızca yöneticiler bu emrden isdifade  edəbilər!__")
...
async def tektag(event):
  ...
  async for admin in client.iter_participants(event.chat_id, filter=ChannelParticipantsAdmins):
    admins.append(admin.id)
  if not event.sender_id in admins:
    return await event.respond(f"Yalnizca Adminler Tag Ede Biler")
